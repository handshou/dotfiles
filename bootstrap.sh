...
echo "  2. UPDATE NVIM SUBMODULE"
echo "  ────────────────────────"
echo ""
echo "     config submodule update --remote --merge"
echo ""
echo "     If conflicts with lazy-lock.json:"
echo "       cd ~/.config/nvim && git checkout lazy-lock.json && cd ~"
echo "       config submodule update --remote --merge"
echo ""
echo "  3. SETUP NEOVIM PLUGINS"
echo "  ───────────────────────"
echo ""
echo "     Step 1: Clear lazy cache (if upgrading neovim or plugins fail):"
echo "       rm -rf ~/.local/share/nvim/lazy ~/.cache/nvim"
echo ""
echo "     Step 2: Open neovim to install plugins:"
echo "       nvim"
echo ""
echo "     Step 3: Wait for Lazy to install plugins, then run:"
echo "       :Lazy sync"
echo ""
echo "  4. SETUP ALACRITTY"
echo "  ──────────────────"
echo ""
...
echo "  ──────────────"
echo ""
echo "     Step 1: Disable SIP (requires Recovery Mode - one time only):"
echo "       - Shut down Mac"
echo "       - Hold Power button until 'Loading startup options' appears"
echo "       - Click Options → Continue"
echo "       - Menu bar: Utilities → Terminal"
echo "       - Run: csrutil enable --without fs --without debug --without nvram"
echo "       - Run: reboot"
echo ""
echo "     Step 2: After reboot, verify SIP status:"
echo "       csrutil status"
echo "       # Should show fs and debug disabled"
echo ""
echo "     Step 3: (asmvik fork only) Set arm64e preview ABI nvram boot-arg:"
echo "       sudo nvram boot-args=\"-arm64e_preview_abi\""
echo "       sudo reboot"
echo "       # Required by asmvik/yabai for --load-sa scripting addition."
echo "       # Verify after reboot: nvram boot-args"
echo "       # Skip this step if you're using koekeishiya/yabai from homebrew-core."
echo ""
echo "     Step 4: Start yabai service (creates plist and starts yabai):"
echo "       yabai --start-service"
echo ""
echo "     Step 5: Grant Accessibility & Screen Recording permissions:"
...
echo ""
echo "     Step 6: Generate sudoers entry (copy the ENTIRE output including username):"
echo "       # NOTE: regenerate this line every time yabai is upgraded — the"
echo "       # SHA256 changes on each new binary, invalidating the prior entry."
...
echo ""
echo "     Step 7: Edit sudoers file and paste the FULL line:"
echo "       sudo visudo -f /private/etc/sudoers.d/yabai"
echo ""
echo "     Step 8: Restart yabai and load scripting addition:"
...
echo "  ───────────────────────"
echo ""
echo "     Step 1: Verify TPM and bash 5+ are installed:"
echo "       brew install tpm bash"
echo "       which bash    # must report /opt/homebrew/bin/bash, not /bin/bash"
echo ""
echo "       (tokyo-night-tmux uses 'declare -A' associative arrays, which"
echo "        macOS's stock bash 3.2 does not support. Without Homebrew bash,"
echo "        the entire status bar renders as one orange-yellow color.)"
echo ""
echo "     Step 2: Open tmux:"
echo "       tmux"
echo ""
echo "     Step 3: Press prefix + I to install plugins:"
echo "       Ctrl+a, then Shift+I"
echo ""
echo "     Step 4: Wait for 'TMUX environment reloaded' message"
echo ""
echo "     Step 5: If status bar colors look wrong, restart the tmux server"
echo "             so plugin scripts pick up the Homebrew bash on PATH:"
echo "       tmux kill-server && tmux"
echo ""
echo "  8. INSTALL NODE.JS VIA NVM"
echo "  ──────────────────────────"
echo ""
echo "     source ~/.nvm/nvm.sh && nvm install --lts"
echo ""
echo "  9. RESTART TERMINAL"
echo "  ───────────────────"
echo ""
